...
#' )
#'
DEMAP <- function(LogPostLike, control_params = AlgoParamsDEMAP(), ...) {

  # import values we will reuse throughout process
  # create memory structures for storing posterior samples
  theta <- array(NA, dim = c(control_params$n_iters_per_chain, control_params$n_chains, control_params$n_params))
  log_post_like <- matrix(-Inf, nrow = control_params$n_iters_per_chain, ncol = control_params$n_chains)
  ...
  message("running DE to find MAP")
  thetaIdx <- 1
  for (iter in 1:control_params$n_iter) {
    if (control_params$parallel_type == "none") {
      # cross over step
      temp <- matrix(unlist(lapply(1:control_params$n_chains, CrossoverOptimize,
